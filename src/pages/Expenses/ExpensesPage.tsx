...
const ExpensesPage: React.FC = () => {
    ...
    try {
      ...
      if (categories.length === 0) {
        ...
        for (const category of defaultCategories) {
          try {
            await expenseCategoriesAPI.create(category);
        ...
        const newCategoriesData = await expenseCategoriesAPI.getAll();
        let newCategories = [];
        if (Array.isArray(newCategoriesData)) {
          newCategories = newCategoriesData;
        } else if (newCategoriesData && newCategoriesData.data && Array.isArray(newCategoriesData.data)) {
          newCategories = newCategoriesData.data;
        }
        setCategories(newCategories);
        console.log('🔍 Updated categories after creating defaults:', newCategories);
      }
    } catch (err) {
  ...
  // Debug categories and form state
  useEffect(() => {
    console.log('🔍 Categories state updated:', categories);
    console.log('🔍 Create form state:', createForm);
